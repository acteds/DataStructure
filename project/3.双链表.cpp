...
typedef struct node
{int data;
struct node *next,*prior;}SL;

int insert(SL *L,const int n,const int k){//顺序写入与插入
if(k<1)return 0;
SL *p=L,*q;
int i=1;
...
int main(){
SL *L=(SL *)malloc(sizeof(SL));//创建双链表
L->next=L->prior=NULL;//初始化
insert(L,2,1);
insert(L,3,2);
insert(L,4,3);
insert(L,1,4);
